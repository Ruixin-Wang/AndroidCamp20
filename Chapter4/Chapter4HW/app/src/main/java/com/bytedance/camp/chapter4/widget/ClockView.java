...
public class ClockView extends View {
    ...
    private List<Integer> numberDegrees = new ArrayList();
    ...
    private void drawTimeNeedles(Canvas canvas) {
         ...
         */
        Log.i("TIME_HOUR", ""+hour);
        Log.i("TIME_MIN", ""+minute);
        Log.i("TIME_SEC", ""+second);

        int hourDegree = hour * 30 + minute / 2  - 90;
        int minDegree = minute * 6 + second / 10 - 90;
        int secDegree = second * 6 - 90;
        float startX, startY, endX, endY;
        startX = centerX;
        startY = centerY;
        endX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hourDegree)));
        endY = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hourDegree)));
        ...
        endY = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minDegree)));
        needlePaint.setAlpha(HIGHLIGHT_UNIT_ALPHA);
        ...
        endY = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secDegree)));
        needlePaint.setAlpha(HIGHLIGHT_UNIT_ALPHA);
    ...
    private void drawTimeNumbers(Canvas canvas) {
        // TODO 绘制表盘时间数字（可选）
        float x, y;
        Paint.FontMetrics Font = numberPaint.getFontMetrics();
        float top = Font.top;
        float bottom = Font.bottom;
        float FontHeight = top - bottom;
        for (int i = 0; i < numberDegrees.size(); i++) {
            x = (int) (centerX + (radius * (1 - 0.2f)) * Math.cos(Math.toRadians(numberDegrees.get(i))));
            y = (int) (centerY - FontHeight*0.3+ (radius * (1 - 0.2f)) * Math.sin(Math.toRadians(numberDegrees.get(i))));
            canvas.drawText("" + (i+1), x, y, numberPaint);
        }
    }
